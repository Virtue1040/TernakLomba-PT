...
<x-layouts.default footer=false>
    ...
    <div class="flex justify-center items-center w-[70%] mx-auto">
        <div x-data="{ step: 1, stepMax: 3 }" class="flex flex-col gap-[58px] p-8 px-0 w-full max-w-[1107px] mt-8 items-center">
            <x-cards.step>
            ...
            <form method="POST" action="{{ route('usersDetail-store') }}" @submit.prevent="step === stepMax + 1 && $el.submit()" class="flex flex-col gap-[42px] w-full" novalidate>
                ...
                <div x-show="step === 1" class="flex flex-col gap-[42px]" name="about_you">
                    ...
                    <div class="flex gap-[44px]">
                        <div class="flex flex-col gap-[36px] w-full">
                            <x-input :required=true name="full_name" label="{!! __('global.full_name') !!}" type="text" placeholder="{!! __('profiling.placeholder_full_name') !!}"/>
                            <x-input :required=true name="birth_date" label="{!! __('global.birth_date') !!}" type="date" placeholder="MM/DD/YYYY"/>
                            <x-input :required=true name="telp" label="{!! __('global.tel') !!}" type="tel" placeholder="+62 08xxxxxx"/>
                        </div>
                        <div class="flex flex-col gap-[36px] w-full">
                            <x-select :required=true name="gender" label="{!! __('global.gender') !!}" type="text" placeholder="{!! __('profiling.placeholder_gender') !!}">
                                <option value="male">{!! __('global.male') !!}</option>
                                <option value="female">{!! __('global.female') !!}</option>
                            </x-select>
                            <x-input :required=true name="birth_place" label="{!! __('global.birth_place') !!}" type="text" placeholder="{!! __('profiling.placeholder_birth_place') !!}"/>
                        </div>
                    </div>
                </div>
                <div x-show="step === 2" class="flex flex-col gap-[42px]" name="akademik">
                    <div class="flex gap-[44px]">
                        <x-select :required=true name="kampus" label="{!! __('global.campus') !!}" placeholder="{!! __('profiling.placeholder_campus') !!}">
                            @foreach ($decoded as $university)
                                <option value="{{ $university['name'] }}">{{ $university['name'] }}</option>
                            @endforeach
                        </x-select>
                        <x-input :required=true name="jurusan" label="{!! __('global.major') !!}" placeholder="{!! __('profiling.placeholder_major') !!}"/>
                    </div>
                    <div>
                        <x-multi-select :required=true name="minat" label="{!! __('profiling.interests') !!}">
                            <div class="flex gap-[10px] flex-wrap">
                                @foreach ($listMinat as $minat)
                                    <x-minat-choose name="bidang[]" title="{{ $minat->name }}" value="{{ $minat->id_bidang }}"/>
                                @endforeach
                            </div>
                        </x-multi-select>
                    </div>
                ...
                <div x-show="step >= 3 " class="flex flex-col gap-[42px] h-full" name="keamanan_akun">
                    <div class="flex gap-[30px] h-fit w-[1107px]">
                        <div class="flex flex-col gap-[20px] w-full border-r-[1px] border-r-[#C6C6C6]">
                           <x-detail type="input" name="full_name" label="{!! __('global.full_name') !!}" text="Budi Setiawan"/>
                        ...
                        <div class="flex flex-col gap-[20px] w-full">
                            <x-detail type="select" name="kampus" label="{!! __('global.campus') !!}" text="Telkom University"/>
                            <x-detail name="jurusan" label="{!! __('global.major') !!}" text="Ilmu Pengetahuan Alam (IPA)"/>
                            <x-detail name="bidang" label="{!! __('global.interest') !!}" text="Business Case, Business Plan, Hackathon UI/UIX"/>
                            <x-detail name="referral_code" label="Referral Code (Opsional)" text="-"/>
                        </div>  
                    </div>
                </div>
                <div class="flex gap-[28px] justify-end">
                    <button x-show="step > 1" @click="step > 1 ? step-- : ''" class="w-[144px] text-[20px] border-[1px] border-[#C6C6C6] h-[68px] text-black rounded-[10px] transition duration-300">{!! __('global.back') !!}
                    </button>
                    <button x-text="step >= stepMax ? '{!! __('global.confirm') !!}' : '{!! __('global.next') !!}'" @click="if (step <= stepMax) { step++ } next(step, stepMax)" class="w-[180px] text-[20px] font-manrope py-[22px] px-[39px] bg-gradient-to-b from-[#822bf2] to-[#b378ff] border-none h-[68px] text-white rounded-[10px] hover:bg-purple-700 transition duration-300">{!! __('global.next') !!}
                    </button>
                </div>
            </form>
        </div>
