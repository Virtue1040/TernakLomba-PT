@props(['name', 'type' => 'text', 'max' => '', 'label' => 'holder', 'placeholder' => 'placeholder here...', 'required' => false, 'tip'])
<?php
...
<div class="flex relative flex-col gap-[10px] w-full">
    ...
    <input id="{{ "input_" . $name}}" name="{{ $name }}" value="{{ old($name) }}" max="{{ $max }}" {{ $attributes->merge(['class' => $class]) }} type="{{ $type }}" placeholder="{{ $placeholder }}">
    @error($name)
